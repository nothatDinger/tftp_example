...
int TFTPClient::UDPconnectServer(){
    ...
    int len = sizeof(client_addr_);
    bzero(&client_addr_, sizeof( client_addr_));
    if(bind(socket_descriptor, (sockaddr*)&client_addr_, sizeof(client_addr_)) < 0 ){
        logger->ERROR("Unable to bind local port");
        return -1;
    }
    bzero(&client_addr_, sizeof(client_addr_));
    
	if (getsockname(socket_descriptor, (struct sockaddr *)&client_addr_, (socklen_t *)&len) == -1) {
    ...
    bzero(&client_addr_, sizeof(client_addr_));
	client_addr_.sin_family = host->h_addrtype;
	client_addr_.sin_port = htons(this->server_port);
	memcpy(&client_addr_.sin_addr, (struct in_addr *) host->h_addr, sizeof(client_addr_.sin_addr));
    this->client_addr = &client_addr_;
